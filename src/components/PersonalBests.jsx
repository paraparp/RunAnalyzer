...
function OverDistance({ record, candidate, t, inline = false }) {
  ...
  return inline
    ? <span className="ml-1 text-slate-300 dark:text-slate-600" title={title}>{label}</span>
    : (
      <span className="text-[9px] font-bold uppercase tracking-wide text-slate-400 bg-slate-100 dark:bg-slate-800 rounded px-1.5 py-0.5" title={title}>
        {label}
      </span>
    );
}
...
function DistanceRecord({ record, t }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200/80 bg-white shadow-sm hover:shadow-md transition-shadow overflow-hidden dark:border-slate-800 dark:bg-slate-900">
      {/* PR row */}
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-2 min-w-0">
            <TrophyIconSolid className="w-4 h-4 text-amber-400 shrink-0" />
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">{record.name}</span>
          </div>
        ...
        <div className="flex items-baseline gap-3 mb-1.5">
          <span className="text-3xl font-black text-slate-900 dark:text-slate-50 tabular-nums leading-none">
          ...
          {pr.isFlat ? (
            <span
              className="text-[9px] font-bold uppercase tracking-wide text-emerald-700 bg-emerald-50 dark:bg-emerald-950/60 dark:text-emerald-300 rounded px-1.5 py-0.5"
              title={pr.gain != null
            ...
            <span className="text-[9px] font-bold uppercase tracking-wide text-violet-600 bg-violet-50 dark:bg-violet-950/60 dark:text-violet-300 rounded px-1.5 py-0.5">
        ...
        <a
          ...
          className="inline-flex items-center gap-1 text-[11px] font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 hover:underline transition-colors truncate max-w-full"
          title={pr.name}
        >
          {pr.name}
        </a>
      ...
      {rest.length > 0 && (
        <>
          <button
            onClick={() => setOpen(o => !o)}
            className="w-full flex items-center justify-between px-4 py-2 bg-slate-50 hover:bg-slate-100 border-t border-slate-100 transition-colors cursor-pointer dark:bg-slate-800/60 dark:hover:bg-slate-800 dark:border-slate-800"
          >
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {rest.map((a, i) => (
                <div key={a.id} className="flex items-center gap-3 px-4 py-2.5">
                  {/* Position badge */}
                  <span className={`text-[10px] font-black w-5 text-center ${MEDAL_COLORS[i] ?? 'text-slate-300 dark:text-slate-600'}`}>
                    {i === 0 ? '🥈' : i === 1 ? '🥉' : `#${i + 2}`}
                  </span>
                  <div className="flex-1 min-w-0">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="block text-[11px] font-medium text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 truncate transition-colors"
                      title={a.name}
                    >
                      {a.name}
                    </a>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100 tabular-nums">
                      ...
                      {a.isEffort && <span className="ml-1 text-[9px] font-bold text-violet-500 dark:text-violet-400 align-middle">✦</span>}
                    </p>
